...
import { push } from "./ioj-api";
// import { pushBinary } from './executable';

const convert = new AnsiToHtml({ escapeXML: true });

export async function compile(_task: RPCRequest): Promise<CompilationResult> {
  const task: CompileTask = _task.task;
  ...
  try {
    ...
    const volumeFile = await pushBinary(
      task.binaryName,
      language,
      task.code,
      binDir
